...
def associate_detections_to_tracks(tracks, detections, threshold, aw_off, grid_off,mot_off, det_embs=None, det_app = False, adaptive_w_assoc_emb=None):
    threshold = 0.3
    ...
    if not det_app:
        if grid_off:
            ...
            emb_cost = emb_cost.T
        # 使用自适应外观权重（如果提供），否则使用默认值0.75
        w_assoc_emb = adaptive_w_assoc_emb if adaptive_w_assoc_emb is not None else 0.75
        aw_param = 0.4
...
def associate_detections_to_trackers_fusion(detections, trackers, aw_off, grid_off,mot_off, iou_threshold, det_embs=None, det_app = False, adaptive_w_assoc_emb=None):
    """
