...
def datafusion2Dand3D(detections_3D_camera, detection_2D, detection_3Dto2D, additional_info,dets_2d_conf):
    '''
    输出的数据
    :param detections_3D_camera:  Detections by LiDAR 3D检测的3D框
    :param detection_2D:          Detections by Camera 2D检测的2D框
    :param detection_3Dto2D:      Detections by LiDAR 3D检测的2D框
    :return:
    ...
    unmatched_detections_2D_conf,matched_conf = [],[]
    for d, det in enumerate(detection_2D):
        if d not in matched_indices[:, 0]:
            unmatched_detections_2D.append(d)
            unmatched_detections_2D_conf.append(dets_2d_conf[d])
    ...
    for m in matched_indices:
        if iou_matrix[m[0], m[1]] < iou_threshold:
            ...
            matched_conf.append(dets_2d_conf[m[0]])
    ...
    return np.array(detection_2D_fusion), np.array(detection_3Dto2D_fusion), detection_3D_fusion, \
               np.array(detection_2D_only), np.array(detection_2Dto3D_only), detection_3D_only,unmatched_detections_2D_conf,matched_conf
