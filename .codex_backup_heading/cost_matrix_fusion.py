...
def compute_fused_cost_matrix(
    ...
    if angle_config is not None and angle_config.enable_angle_feature:
        try:
            ...
            sigma_value = angle_config.angle_cost_sigma
            if getattr(angle_config, 'enable_speed_adaptive_sigma', False):
                track_speeds = []
                for track in tracks:
                    try:
                        track_speeds.append(float(np.linalg.norm(get_velocity(track)[:3])))
                    except Exception:
                        track_speeds.append(0.0)
                sigma_value = np.array([
                    compute_speed_adaptive_sigma(
                        speed=speed,
                        sigma_base=angle_config.angle_cost_sigma,
                        sigma_min=getattr(angle_config, 'speed_sigma_min', None),
                        sigma_max=getattr(angle_config, 'speed_sigma_max', None),
                        sigma_k=getattr(angle_config, 'speed_sigma_k', 0.25),
                    )
...
def get_track_angle(track):
    # Prefer the smoothed heading used elsewhere in matching to keep angle semantics consistent.
    if hasattr(track, 'angle_smoothed') and track.angle_smoothed is not None:
        return track.angle_smoothed
    """从轨迹对象中提取角度 (优先3D yaw)"""
    # 优先 Track_3D.pose[6]
