...
class KalmanFilterNew(object):
    ...
    **Example**
    ...
    dim_x : int
        ...
        卡尔曼滤波器的状态变量数。例如，如果要在二维空间中跟踪物体的位置和速度，则dim_x为 4。这用于设置 P、Q 和 U 的默认大小
    dim_z : int
        Number of of measurement inputs. For example, if the sensor
        provides you with position in (x,y), dim_z would be 2.
        测量输入的数量。例如，如果传感器在 （x，y） 中为您提供位置，则dim_z将为 2。
    dim_u : int (optional)
    ...
    def update(self, z, R=None, H=None, new_kf=False):
        ...
        将新的测量值 （z） 添加到卡尔曼滤波器。如果 z 为 None，则不计算任何内容。
        但是，x_post 和 P_post 会使用前一个 （x_prior， P_prior） 进行更新，并且 self.z 设置为 None。
        Parameters
        ----------
