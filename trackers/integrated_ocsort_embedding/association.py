...
def speed_direction_batch(dets, tracks):
    '''
    计算检测到的对象相对于跟踪的对象的速度和方向。
    CX,CY 为检测框的中心坐标
    dx,dy是标准化过后的坐标
    '''
    tracks = tracks[..., np.newaxis]  # 在 tracks 数组的最后一个维度上增加一个维度，使其形状变为 (num_track, 5, 1)
...
def associate(
    ...
    Y, X = speed_direction_batch(detections, previous_obs)  # 计算运动检测结果的运动方向
    inertia_Y, inertia_X = velocities[:, 0], velocities[:, 1]
    inertia_Y = np.repeat(inertia_Y[:, np.newaxis], Y.shape[1], axis=1)  # 重复初始的值，匹配上Y,X的维度
    inertia_X = np.repeat(inertia_X[:, np.newaxis], X.shape[1], axis=1)
    diff_angle_cos = inertia_X * X + inertia_Y * Y    # 计算运动方向的余弦相似度，然后将其转换为角度差异度量。
    diff_angle_cos = np.clip(diff_angle_cos, a_min=-1, a_max=1)
    diff_angle = np.arccos(diff_angle_cos)
    diff_angle = (np.pi / 2.0 - np.abs(diff_angle)) / np.pi

    valid_mask = np.ones(previous_obs.shape[0])  # 创建一个有效掩码，将无效检测（标签为负数）的对应位置设置为0。
    valid_mask[np.where(previous_obs[:, 4] < 0)] = 0

    iou_matrix = iou_batch(detections, trackers)
    scores = np.repeat(detections[:, -1][:, np.newaxis], trackers.shape[0], axis=1)  # 检测结果的置信度
    # iou_matrix = iou_matrix * scores # a trick sometiems works, we don't encourage this
    valid_mask = np.repeat(valid_mask[:, np.newaxis], X.shape[1], axis=1)

    # 计算运动方向差异的代价，考虑到有效掩码、分数和权重。
    angle_diff_cost = (valid_mask * diff_angle) * vdc_weight
    angle_diff_cost = angle_diff_cost.T # 转置
    angle_diff_cost = angle_diff_cost * scores

    emb_cost = None
    if not emb_off:
        if grid_off:
            emb_cost = None if (trk_embs.shape[0] == 0 or det_embs.shape[0] == 0) else det_embs @ trk_embs.T   # @数组运算
        else:
    ...
    if min(iou_matrix.shape) > 0:
        a = (iou_matrix > iou_threshold).astype(np.int32)
        if a.sum(1).max() == 1 and a.sum(0).max() == 1:  # 判断每行每列是否只有一个匹配项
            matched_indices = np.stack(np.where(a), axis=1) # 将a中值为1的位置的行列索引堆叠在一起，生成一个形状为(num_matches,2)的数组，其中每行包含一个匹配对的行和列索引。
        else:
            if emb_cost is None:
                emb_cost = 0
            else:
            ...
            matched_indices = linear_assignment(final_cost)  # 匈牙利匹配
    else:
        matched_indices = np.empty(shape=(0, 2))
